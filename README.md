...
Since you're here looking at this code, you're probably thinking: "This sounds great for a prototype but how the heck am I going to maintain and enhance this thing?"  Well, we focus on simplicity and modularity... and you're reading the code!  We have the following modules:
 ...
 - *Sheety Core Presenters* - Core presentation components used in sheety apps.
...
## Test
```bash
  docker-compose run --rm sheety-presenters npm test
```

## Add dependencies
```bash
  docker-compose run --rm sheety-presenters yarn add <dependency>
```

## Build for production
``` bash
  docker-compose run --rm sheety-presenters npm run build
```
