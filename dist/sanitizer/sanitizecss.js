...
exports.sanitizeMediaQuery = exports.sanitizeStylesheet = exports.sanitizeCssSelectorList = exports.sanitizeCssProperty = undefined;
...
var _csspropbit = require('./csspropbit');

var cssPropBits = _interopRequireWildcard(_csspropbit);

var _csslexer = require('./csslexer');
...
(function () {
  ...
  // From RFC3986
  var URI_SCHEME_RE = new RegExp('^' + '(?:' + '([^:\/?# ]+)' + // scheme
  ':)?');
  ...
  function resolveUri(baseUri, uri) {
    if (baseUri) {
      return _uri2.default.utils.resolve(baseUri, uri);
    }
    return uri;
  }
  ...
  function withoutVendorPrefix(ident) {
    ...
    return ident.replace(/^-(?:apple|css|epub|khtml|moz|mso?|o|rim|wap|webkit|xv)-(?=[a-z])/, '');
  }
   ...
   */
  exports.sanitizeCssProperty = sanitizeCssProperty = function () {
    ...
    return function sanitize(property, tokens, opt_naiveUriRewriter, opt_baseUri, opt_idSuffix) {
      ...
      var propertySchema = cssPropBits.default[propertyKey];

      // If the property isn't recognized, elide all tokens.
      if (!propertySchema || 'object' !== (typeof propertySchema === 'undefined' ? 'undefined' : _typeof(propertySchema))) {
        tokens.length = 0;
        return;
      }
        ...
        if (!parenDepth) {
          ...
          for (var i = 0, nFns = fns.length; i < nFns; ++i) {
            if (fns[i].substring(0, bareFnToken.length) == bareFnToken) {
              ...
              fnTokens, opt_naiveUriRewriter, opt_baseUri);
      ...
      var stringDisposition = propBits & (cssPropBits.CSS_PROP_BIT_URL | cssPropBits.CSS_PROP_BIT_UNRESERVED_WORD);
      // Used to determine what to do with unreserved words.
      var identDisposition = propBits & (cssPropBits.CSS_PROP_BIT_GLOBAL_NAME | cssPropBits.CSS_PROP_BIT_PROPERTY_NAME);
      ...
      for (; i < tokens.length; ++i) {
        ...
        cc === ' '.charCodeAt(0) ? '' : cc === '"'.charCodeAt(0) ? // Quoted string.
        stringDisposition === cssPropBits.CSS_PROP_BIT_URL ? opt_naiveUriRewriter
        ...
        resolveUri(opt_baseUri,
        // Strip off quotes
        (0, _csslexer.decodeCss)(tokens[i].substring(1, token.length - 1))), propertyKey, opt_naiveUriRewriter)) : '' : propBits & cssPropBits.CSS_PROP_BIT_QSTRING &&
        // Ambiguous when more than one bit set in disposition.
        !(stringDisposition & stringDisposition - 1) ? token
        // Drop if quoted strings not allowed.
        : '' :

        // inherit is always allowed.
        token === 'inherit' ? token : (litGroup = propertySchema['cssLitGroup'], litMap = litGroup ? propertySchema['cssLitMap']
        // Lazily compute the union from litGroup.
        || (propertySchema['cssLitMap'] = unionArrays(litGroup)) : ALLOWED_LITERAL, // A convenient empty object.
        ...
        : cc === '#'.charCodeAt(0) && /^#(?:[0-9a-f]{3,4}){1,2}$/.test(token) ? propBits & cssPropBits.CSS_PROP_BIT_HASH_VALUE ? token : '' : '0'.charCodeAt(0) <= cc && cc <= '9'.charCodeAt(0) ?
        ...
        (cc1 = token.charCodeAt(1), cc2 = token.charCodeAt(2), isnum1 = '0'.charCodeAt(0) <= cc1 && cc1 <= '9'.charCodeAt(0), isnum2 = '0'.charCodeAt(0) <= cc2 && cc2 <= '9'.charCodeAt(0),
        // +.5 -> 0.5 if allowed.
        cc === '+'.charCodeAt(0) && (isnum1 || cc1 === '.'.charCodeAt(0) && isnum2)) ? propBits & cssPropBits.CSS_PROP_BIT_QUANTITY ? (isnum1 ? '' : '0') + token.substring(1) : '' :

        // -.5 -> -0.5 if allowed otherwise -> 0 if quantities allowed.
        cc === '-'.charCodeAt(0) && (isnum1 || cc1 === '.'.charCodeAt(0) && isnum2) ? propBits & cssPropBits.CSS_PROP_BIT_NEGATIVE_QUANTITY ? (isnum1 ? '-' : '-0') + token.substring(1) : propBits & cssPropBits.CSS_PROP_BIT_QUANTITY ? '0' : '' :

        // .5 -> 0.5 if allowed.
        cc === '.'.charCodeAt(0) && isnum1 ? propBits & cssPropBits.CSS_PROP_BIT_QUANTITY ? '0' + token : '' :

        // Handle url("...") by rewriting the body.
        'url("' === token.substring(0, 5) ? opt_naiveUriRewriter && propBits & cssPropBits.CSS_PROP_BIT_URL ? normalizeUrl(safeUri(resolveUri(opt_baseUri, tokens[i].substring(5, token.length - 2)), propertyKey, opt_naiveUriRewriter)) : '' :

        // Handle func(...) by recursing.
        // Functions start at a token like "name(" and end with a ")" taking
        // into account nesting.
        token.charAt(token.length - 1) === '(' ? sanitizeFunctionCall(tokens, i) : identDisposition && /^-?[a-z_][\w\-]*$/.test(token) && !/__$/.test(token) ? opt_idSuffix && identDisposition === cssPropBits.CSS_PROP_BIT_GLOBAL_NAME ? tokens[i] + opt_idSuffix // use original token, not lowercased
        : identDisposition === cssPropBits.CSS_PROP_BIT_PROPERTY_NAME && cssPropBits.default[token] && 'number' === typeof cssPropBits.default[token].cssPropBits ? token : '' : /^\w+$/.test(token) && stringDisposition === cssPropBits.CSS_PROP_BIT_UNRESERVED_WORD && propBits & cssPropBits.CSS_PROP_BIT_QSTRING ?
        ...
        ? (tokens[lastQuoted] = tokens[lastQuoted].substring(0, tokens[lastQuoted].length - 1) + ' ' + token + '"', token = '') : (lastQuoted = k, '"' + token + '"') :
   ...
   */
  exports.sanitizeCssSelectorList = sanitizeCssSelectorList = function sanitizeCssSelectorList(selectors, virtualization, opt_onUntranslatableSelector) {
    ...
    for (i = 0; i < selectors.length; ++i) {
      ...
      if (tok == '(' || tok == '[' ? (++inBrackets, true) : tok == ')' || tok == ']' ? (inBrackets && --inBrackets, true) : !(selectors[i] == ' ' && (inBrackets || COMBINATOR[selectors[i - 1]] === COMBINATOR || COMBINATOR[selectors[i + 1]] === COMBINATOR))) {
    ...
    function processComplexSelector(start, end) {
      ...
      function processCompoundSelector(start, end, combinator) {
        ...
        for (; valid && start < end; ++start) {
          ...
          if (tok.charAt(0) === '#') {
            ...
            if (++start < end && /^[0-9A-Za-z:_\-]+$/.test(tok = selectors[start]) && !/^_|__$/.test(tok)) {
              classId += '.' + tok;
            } else {
              valid = false;
            }
            ...
            var atype = _html2.default.ATTRIBS[element + '::' + vAttr];
            if (atype !== +atype) {
              atype = _html2.default.ATTRIBS['*::' + vAttr];
            }
            ...
            if (virtualization.virtualizeAttrName) {
              ...
              if (valid && atype !== +atype) {
                atype = _html2.default.atype['NONE'];
              }
            } else {
              rAttr = vAttr;
              if (atype !== +atype) {
                // not permitted according to schema
                valid = false;
              }
            }
            ...
            switch (atype) {
              case _html2.default.atype['CLASSES']:
              case _html2.default.atype['LOCAL_NAME']:
              case _html2.default.atype['NONE']:
                break;
              case _html2.default.atype['GLOBAL_NAME']:
              case _html2.default.atype['ID']:
              case _html2.default.atype['IDREF']:
                if ((op === '=' || op === '~=' || op === '$=') && value != '""' && !ignoreCase) {
                  // The suffix is case-sensitive, so we can't translate case
                  // ignoring matches.
                  value = '"' + value.substring(1, value.length - 1) + idSuffix + '"';
                } else if (op === '|=' || op === '') {
                ...
                break;
              case _html2.default.atype['URI']:
              case _html2.default.atype['URI_FRAGMENT']:
            ...
            if (valid) {
              attrs += '[' + rAttr.replace(/[^\w-]/g, '\\$&') + op + value + (ignoreCase ? ' i]' : ']');
            }
        ...
        if (valid) {
          ...
          // be escaped.
          var selector = (element + classId).replace(/[^ .*#\w-]/g, '\\$&') + attrs + pseudoSelector + combinator;
          if (selector) {
            out.push(selector);
          }
        }
      ...
      if (valid) {
        ...
        return !opt_onUntranslatableSelector || opt_onUntranslatableSelector(selectors.slice(start, end));
      }
    }
    return sanitized;
  };
  ...
  (function () {
    var MEDIA_TYPE = '(?:' + 'all|aural|braille|embossed|handheld|print' + '|projection|screen|speech|tty|tv' + ')';
    ...
    var MEDIA_FEATURE = '(?:' + '(?:min-|max-)?' + '(?:' + ('(?:device-)?' + '(?:aspect-ratio|height|width)' + '|color(?:-index)?' + '|monochrome' + '|orientation' + '|resolution') + ')' + '|grid' + '|hover' + '|luminosity' + '|pointer' + '|scan' + '|script' + ')';

    var LENGTH_UNIT = '(?:p[cxt]|[cem]m|in|dpi|dppx|dpcm|%)';

    var CSS_VALUE = '-?(?:' + '[a-z]\\w+(?:-\\w+)*' // An identifier
    ...
    var MEDIA_EXPR = '\\( ' + MEDIA_FEATURE + ' (?:' + ': ' + CSS_VALUE + ' )?\\)';

    var MEDIA_QUERY = '(?:' + '(?:(?:(?:only|not) )?' + MEDIA_TYPE + '|' + MEDIA_EXPR + ')'
    ...
    var MEDIA_QUERY_LIST_REGEXP = new RegExp('^' + MEDIA_QUERY + '(?: , ' + MEDIA_QUERY + ')*' + '$', 'i');
     ...
     */
    exports.sanitizeMediaQuery = sanitizeMediaQuery = function sanitizeMediaQuery(cssTokens) {
      cssTokens = cssTokens.slice();
      // Strip out space tokens.
      var nTokens = cssTokens.length,
          k = 0;
      ...
      css = !css.length ? '' // Always true per the spec.
      : !MEDIA_QUERY_LIST_REGEXP.test(css) ? 'not all' // Always false.
      // Emit as-is if it starts with 'only', 'not' or a media type.
      : STARTS_WITH_KEYWORD_REGEXP.test(css) ? css : 'not all , ' + css // Not ambiguous with a URL.
      ;
      return css;
    };
  })();
  ...
  (function () {
     ...
     */
    function sanitizeStylesheetInternal(baseUri, cssText, virtualization, naiveUriRewriter, naiveUriFetcher, continuation, opt_importCount) {
      var safeCss = void 0;
      ...
      var elide = false;
      (0, _cssparser.parseCssStylesheet)(cssText, {
        'startStylesheet': function startStylesheet() {
          safeCss = [];
        },
        'endStylesheet': function endStylesheet() {},
        'startAtrule': function startAtrule(atIdent, headerArray) {
          if (elide) {
            atIdent = null;
          } else if (atIdent === '@media') {
            safeCss.push('@media', ' ', sanitizeMediaQuery(headerArray));
          } else if (atIdent === '@keyframes' || atIdent === '@-webkit-keyframes') {
            var animationId = headerArray[0];
            if (headerArray.length === 1 && !/__$|[^\w\-]/.test(animationId)) {
              safeCss.push(atIdent, ' ', animationId + virtualization.idSuffix);
              atIdent = '@keyframes';
            } else {
              atIdent = null;
            }
            ...
            if (atIdent === '@import' && headerArray.length > 0) {
              ...
              if ('function' === typeof continuation) {
                ...
                if (mediaQuery !== 'not all') {
                  ...
                  var cssUrl = safeUri(resolveUri(baseUri, cssParseUri(headerArray[0])), function (result) {
                    var sanitized = sanitizeStylesheetInternal(cssUrl, result.html, virtualization, naiveUriRewriter, naiveUriFetcher, continuation, importCount);
                    --importCount[0];
                    var safeImportedCss = mediaQuery ? {
                      toString: function toString() {
                        return '@media ' + mediaQuery + ' {' + sanitized.result + '}';
                      }
                    } : sanitized.result;
                ...
                if (window.console) {
                  window.console.log('@import ' + headerArray.join(' ') + ' elided');
                }
              }
            }
          }
        ...
        'startRuleset': function startRuleset(selectorArray) {
          if (!elide) {
            var selector = void 0;
            if (blockStack[blockStack.length - 1] === '@keyframes') {
              // Allow [from | to | <percentage>]
              selector = selectorArray.join(' ').match(/^ *(?:from|to|\d+(?:\.\d+)?%) *(?:, *(?:from|to|\d+(?:\.\d+)?%) *)*$/i);
              elide = !selector;
              if (selector) {
                selector = selector[0].replace(/ +/g, '');
              }
            } else {
              var selectors = sanitizeCssSelectorList(selectorArray, virtualization);
        ...
        'declaration': function declaration(property, valueArray) {
          if (!elide) {
            var isImportant = false;
            var nValues = valueArray.length;
            if (nValues >= 2 && valueArray[nValues - 2] === '!' && valueArray[nValues - 1].toLowerCase() === 'important') {
              isImportant = true;
              valueArray.length -= 2;
            }
            sanitizeCssProperty(property, valueArray, naiveUriRewriter, baseUri, virtualization.idSuffix);
            if (valueArray.length) {
              safeCss.push(property, ':', valueArray.join(' '), isImportant ? ' !important;' : ';');
            }
          }
        }
      });
    ...
    exports.sanitizeStylesheet = sanitizeStylesheet = function sanitizeStylesheet(baseUri, cssText, virtualization, naiveUriRewriter) {
      return sanitizeStylesheetInternal(baseUri, cssText, virtualization, naiveUriRewriter, undefined, undefined).result.toString();
    };

    sanitizeStylesheetWithExternals = function sanitizeStylesheetWithExternals(baseUri, cssText, virtualization, naiveUriRewriter, naiveUriFetcher, continuation) {
      return sanitizeStylesheetInternal(baseUri, cssText, virtualization, naiveUriRewriter, naiveUriFetcher, continuation);
    };
  })();
})();

exports.sanitizeCssProperty = sanitizeCssProperty;
exports.sanitizeCssSelectorList = sanitizeCssSelectorList;
exports.sanitizeStylesheet = sanitizeStylesheet;
exports.sanitizeMediaQuery = sanitizeMediaQuery;
//# sourceMappingURL=sanitizecss.js.map