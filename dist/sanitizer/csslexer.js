...
(function () {
  ...
  function escapeCssStrChar(ch) {
    return cssStrChars[ch] || (cssStrChars[ch] = '\\' + ch.charCodeAt(0).toString(16) + ' ');
  }
  ...
  function escapeCssUrlChar(ch) {
    return cssUrlChars[ch] || (cssUrlChars[ch] = (ch < '\x10' ? '%0' : '%') + ch.charCodeAt(0).toString(16));
  }
  ...
  var ESCAPE_TAIL = '(?:' + UNICODE_TAIL + '|[\\u0020-\\u007e\\u0080-\\ud7ff\\ue000\\ufffd]|' + SURROGATE_PAIR + ')';
  var ESCAPE = '\\\\' + ESCAPE_TAIL;
  // urlchar  ::=  [#x9#x21#x23-#x26#x28-#x7E] | nonascii | escape
  var URLCHAR = '(?:[\\t\\x21\\x23-\\x26\\x28-\\x5b\\x5d-\\x7e]|' + NONASCII + '|' + ESCAPE + ')';
  ...
  var STRING = '"(?:\'|' + STRINGCHAR + ')*"' + '|\'(?:\"|' + STRINGCHAR + ')*\'';
  // num  ::=  [0-9]+ | [0-9]* '.' [0-9]+
  ...
  var CSS_TOKEN = new RegExp([BOM, UNICODE_RANGE, URI, FUNCTION, WORD_TERM, STRING, NUMERIC_VALUE, CDO, CDC, S, COMMENT, CMP_OPS, CHAR].join("|"), 'gi');
   ...
   */
  exports.decodeCss = decodeCss = function decodeCss(css) {
   ...
   */
  exports.lexCss = lexCss = function lexCss(cssText) {
    ...
    for (var i = 0, n = tokens.length; i < n; ++i) {
      ...
      cc == '"'.charCodeAt(0) || cc == '\''.charCodeAt(0) ? escapeCssString(tok.substring(1, len - 1), escapeCssStrChar)
      ...
      : /url\(/i.test(tok) ? 'url(' + escapeCssString(tok.replace(URL_RE, ''), escapeCssUrlChar) + ')'
...
exports.lexCss = lexCss;
exports.decodeCss = decodeCss;
//# sourceMappingURL=csslexer.js.map